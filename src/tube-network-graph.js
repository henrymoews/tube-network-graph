...
// the first two requires are necessary for async/await
...
const checkGraphviz = async () => {
  const graphviz_check = shell.exec(`which ${GRAPHVIZ_COMMAND}`, {silent: true});
  if (graphviz_check.code !== 0) {
    return Promise.reject(`Couldn't find command '${GRAPHVIZ_COMMAND}'. Is Graphviz installed and in $PATH?`);
  }
...
const makeDotFileText = (processedData, includeStopPosition, includeStopSize) => {
  ...
  Object.values(processedData.stops).forEach(stop => {
    dotLines.push(`${stop.id} [xlabel="${stop.name}"];`)
  });
...
const run = async () => {
  try {
    ...
    new shell.ShellString(dotFileText).to('tube.gv');
